...
public class Service implements ServiceRemote, ServiceLocal {
	...
	public Categorie getCategorieById(int id) throws ServiceException{
		Categorie ca=null;
		try {
			ca=categorieDAO.getCatById(id);
		} catch (Exception e) {
			throw new ServiceException("Database Categorie : GetById Error");
		}
		return ca;
	}
	...
	@Override
	public void updatePiece(Piece piece) throws ServiceException {
		try {
			pieceDAO.update(piece);
		} catch (Exception e) {
			logger.error("logger error : Piece update failed)");
			throw new ServiceException(e.getMessage());
		}
	}
...
/*--------------------------------------Commande--------------------------------------------*/
	...
	@Override
	public Set<Commande> commandeGetByUtilisateur (Utilisateur util) throws ServiceException{
		Set<Commande> commandes=null;
		try{
			commandes = utilisateurDAO.commandesGetByUtilisateur(util);
		}
		catch(Exception e){
			logger.error("logger error : Utilisateur failed getCommandes");
			throw new ServiceException(e.getMessage());
		}
		return commandes;
	}
